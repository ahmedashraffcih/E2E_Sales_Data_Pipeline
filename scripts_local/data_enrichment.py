...
import re
import random
from scripts_local.utils.utils import *
import logging
...
def process_users_data():
    ...
    try:
        ...
        existing_users_df["start_date"] = pd.to_datetime(
            existing_users_df["start_date"]
        )
    ...
    for idx, new_row in users_df.iterrows():
        existing_row = existing_users_df[
            existing_users_df["user_id"] == new_row["user_id"]
        ]
        ...
        if not existing_row.empty:
            ...
            # Check for changes
            if (
                (new_row["name"] != existing_row["name"])
                or (new_row["email"] != existing_row["email"])
                or (new_row["city"] != existing_row["city"])
            ):

                # Update existing record
                existing_users_df.loc[
                    existing_users_df["user_id"] == new_row["user_id"], "end_date"
                ] = datetime.now().date()
                existing_users_df.loc[
                    existing_users_df["user_id"] == new_row["user_id"], "is_current"
                ] = False
                # Add the new record
